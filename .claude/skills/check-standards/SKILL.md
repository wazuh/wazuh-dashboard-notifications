...
> **repo-specific (wazuh-dashboard-notifications):** this repo **has its own local
...
> **repo-specific (wazuh-dashboard-notifications):** ESLint config is `.eslintrc.js`
> (extends `@elastic/eslint-config-kibana` + `plugin:@elastic/eui/recommended`; a
> legacy YAML `.eslintrc` is also present), and the lint script runs through the
> parent checkout. `yarn lint` runs **both ESLint and Stylelint**
> (`node ../../scripts/eslint . && node ../../scripts/stylelint`). Run it from this
> plugin's dir inside `wazuh-dashboard/plugins/<this-plugin>`. It enforces the
> short SPDX **license header** on `.js`/`.ts`/`.tsx` files, so new files need the
...
> **repo-specific (wazuh-dashboard-notifications):** there is **no `typecheck`
...
> **repo-specific (wazuh-dashboard-notifications):** the test script is
> **`yarn test:jest`** (it runs `TZ=UTC ../../node_modules/.bin/jest --config
...
> only when the change intends to (`yarn test:jest -u`).
