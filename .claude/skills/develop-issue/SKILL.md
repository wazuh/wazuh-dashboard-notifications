...
- **Internal** — URL contains `internal-devel-request` (e.g.
  ...
  (omit the closing keyword in `## Description`) and there is **no CHANGELOG
  entry**.
- **Public** — any other repo (e.g. `wazuh/wazuh-dashboard-notifications`): link
  it in the PR and add a CHANGELOG entry pointing to the issue.
...
- This is a fork of `opensearch-project/dashboards-notifications`: match the
  **upstream filename convention** (PascalCase for components, e.g.
  `ContentPanel`), TypeScript, English everywhere. New `.js`/`.ts`/`.tsx` files
  need the short SPDX **license header** (`Copyright OpenSearch Contributors` /
  `SPDX-License-Identifier: Apache-2.0`), enforced by ESLint. On upstream syncs,
  Wazuh content wins.
...
> **repo-specific (wazuh-dashboard-notifications):** this plugin's scripts run from
...
> **`yarn test:jest`** here; refresh snapshots with `yarn test:jest -u`.
...
- The filled PR-template body, with the **`## Description`** completed, the
  **`### Review Checklist`** boxes that genuinely apply checked, and the
  **closing keyword omitted** for internal-devel-requests issues (or
  `Closes #<n>` / issue URL for public ones), plus a screenshot/video under
