...
- **Internal** — the URL/repo contains `internal-devel-request` (e.g.
  ...
  - PR `## Description`: **leave out the closing keyword** — never expose the
    internal link.
  - CHANGELOG: **no entry** for internal-devel-requests issues.
- **Public** — any other repo (e.g.
  `https://github.com/wazuh/wazuh-dashboard-notifications/issues/123`):
...
> **repo-specific (wazuh-dashboard-notifications):** the changelog is
...
**verbatim** — read it first, keep every heading exactly, do not restate its
sections here.

Fill each section with real content; check the boxes in `### Review Checklist`
that genuinely apply. Put the screenshot/video (REQUIRED for any UI change)
under `### Results and Evidence`. In `## Description`: public issue → closing
keyword (`Closes`, `Fixes`, `Fix`) with `#<n>` or the full issue URL;
**internal-devel-requests issue → omit the closing keyword** (see "Issue
source" above).
