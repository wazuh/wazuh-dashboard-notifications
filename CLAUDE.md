...
Wazuh-owned AI context for **`wazuh-dashboard-notifications`**. Keep it short: this
...
[`opensearch-project/dashboards-notifications`](https://github.com/opensearch-project/dashboards-notifications).
It provides the **Notifications** UI in the Wazuh dashboard (notification channels
& senders — email/Slack/webhook/etc. — and their related configuration). It is
_not_ the platform — the platform is the sibling repo `wazuh-dashboard` (an OSD
fork), into which this plugin is installed under its external `./plugins/`
directory (alongside the plugins from `wazuh-dashboard-plugins`).

- OSD id: `notificationsDashboards` (`opensearch_dashboards.json`); package name
  `notifications-dashboards`; build zip `notifications-dashboards`. There is no
  explicit `configPath` (it uses the default).
...
3. Cross-plugin access (e.g. to plugins from `wazuh-dashboard-plugins` or built-in
   ...
   `opensearchDashboardsUtils` and optionally `share`, `managementOverview`,
...
yarn lint                 # node ../../scripts/eslint . && node ../../scripts/stylelint (ESLint + Stylelint)
node ../../scripts/eslint . --fix   # autofix ESLint
...
Note the test script is **`yarn test:jest`** and lint runs **both ESLint and
Stylelint**. There is **no** `format`, `lint:fix`, `typecheck`, or `knip` script
here. This repo ships its own `.prettierrc` (`es5` trailing commas, single quotes,
80-col) and `.prettierignore` (skips `*.md`, `*.lock`); a `.lintstagedrc` defines
`prettier --write` on staged files, but **no husky hook is installed** — run
Prettier yourself with `npx prettier <files> --write`. `tsconfig.json` extends
...
`wazuh-dashboard-security-analytics`, `wazuh-dashboard-alerting`) — is the Docker
dev env **owned by `wazuh-dashboard-plugins`** (`docker/osd-dev`). Mount this repo
into it with `-r`:
...
- **Filenames follow the upstream OpenSearch convention** (PascalCase for
  components, e.g. `ContentPanel`, `PageHeader`; snake/camel elsewhere) — match
  the surrounding upstream style when editing.
...
- **Unit tests are colocated** as `*.test.ts` / `*.test.tsx`, typically inside
  ...
  `__snapshots__/`). Run the whole suite with `yarn test:jest`; scope with a path
  (`yarn test:jest <path>`). Refresh snapshots only when a change intends to
  (`yarn test:jest -u`). When you add a source file, add its test beside it.
...
- Issues arrive as URLs and may live in another repo. Issues from
  ...
  (omit the closing keyword in `## Description`) and add no CHANGELOG entry.

## Fork coexistence

Upstream is `opensearch-project/dashboards-notifications`. On upstream syncs,
